class UserController < ApplicationController
  ...
  def feevy
    ...
    cache_key << "_#{params[:tags].gsub(" ", "+")}" if params[:tags]
    ...
    unless @feevy = CACHE.get(cache_key)
      ...
      #pruebaa
      @stats_link     = "http://favs.homelinux.org/stats/" + params[:id].to_s + "/" + params[:tags].to_s      
      @join_link      = "http://favs.homelinux.org/subscriptions/new/" + params[:id].to_s + "/" + params[:tags].to_s      
  ...
  def stats
    ...
    @style =  render_to_string(:partial => partial_style, :locals => { :id => params[:id]})
    @style = "" if params[:style] == "open-css"
    @content = render_to_string(:partial => partial_badge, :locals => { :id => params[:id], :entradas => @posts, :blogs => @blogs, :pupils => @pupils} )
    @stats = [@content, @style]
