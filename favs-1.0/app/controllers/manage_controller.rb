  ...
  def select_blogs
    ...
    if request.post?
      ...
      params[:blogs].each do |blog|
	    ...
	    pupils_name = params[:pupils_name]
	    mail = params[:mail]	
	    feed = Feed.create_from_blog(blog,pupils_name,mail)
            flash[:message] = "Feed has been added to your FAVS."
            if @user.add_subscription(feed).nil?
              flash[:message] = "You've already got this feed in your FAVS"
              @user.update_attribute :registration_stage, 2
            end
          rescue => err
      ...
      if not @subscriptions.detect{ |sub| sub.feed.avatar_locked == 1 }.nil?
        @user.subscriptions_added
        @user.update_attribute :registration_stage, 2
        flash[:message] = "Feeds has been added to your FAVS"
        redirect_to :action => 'index'
      else
      #render :action => 'select_avatars'
      redirect_to :action => 'index'		
...
#############################################
  def select_name
    ...
    if request.post?
      ...
      @user.save		
      redirect_to :action => 'avatar_selected'
    end
  end
...
############################################
  ...
  def display_feevy_code
    ...
    script = "&lt;script type='text/javascript' src='http://favs.homelinux.org/code/#{@user.id}#{style}'&gt;&lt;/script&gt;"
    render :text => script
  end
  ...
  def update_opt_display
    check_user
    @user.update_attribute :opt_display, params[:display_feevy]
    render :nothing => true
  end
