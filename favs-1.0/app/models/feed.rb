class Feed < ActiveRecord::Base
  ...
  def Feed.create_from_blog(input_url,name,mail)
    # clean up the url
    begin
    ...
    if Rfeedfinder::isFeed?(input_url)
      return Feed.create_feed(web_url=nil,feed_url=input_url,pupils_name=name,pupils_mail=mail)
    else
      return Feed.create_feed(web_url=input_url,feed_url=nil,pupils_name=name,pupils_mail=mail)
    end
  end
  ...
  def Feed.create_feed(web_url=nil, feed_url=nil,pupils_name=nil,pupils_mail=nil)

    logger.debug "entra en create_feed"
    return nil if web_url.nil? and feed_url.nil?
    ...
    ## Añado excepcion para blogspot
    if feeding.title.include? "blogspot"
      feed_url = web_url + "feeds/posts/default?alt=rss"
    end
    
    logger.debug "llega al Feed.create"    
  ...
  encontrado = 0
    begin
      ...
      Timeout::timeout(30) do
        ...
        #puts "antes del if entry.title: #{entry.title}" 
        #puts "latest_post: #{latest_post}"  
        ...
        #puts "Entra en el primer if"
	     ...
	     Post.create(:url => entry.link, 
             ...
             #puts "entra en el else"
             historias = posts
             historias.each do |post|
               #puts "comprueba" + entry.title + " contra " + post.title
               #puts "encontrado: " + encontrado
               if  entry.title == post.title or entry.link == post.url
                 encontrado = 1
               end
             end 
             #puts "encontrado: " + encontrado
